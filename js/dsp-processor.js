/** Source copy retained for inspection; the served worklet is in public/. */
'use strict';
class DSPProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this._bitDepth = 8; this._grit = 1; this._noise = 0; this._crush = true; this._dither = true; this._previous = new Float32Array(2);
  ...
  process(inputs, outputs) {
    ...
    const halfLevels = 1 << (this._bitDepth - 1), lsb = 1 / halfLevels;
    for (let ch = 0; ch < output.length; ch++) {
      const source = input[ch], target = output[ch];
